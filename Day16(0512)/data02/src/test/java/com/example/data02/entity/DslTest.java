...
//        between(a,b)
        ...
        list.stream().map(ele -> ele.getId()).forEach(System.out::println);
...
//        and를 다음과 같이도 사용 가능***
        queryFactory.selectFrom(employee)
//                where 내부에서 ,를 사용하여 여러 조건을 넘겨주면 and 처리
...
//        1. fetchOne() : 단 건 조회, 결과가 없으면 null을 반환하고 2개 이상 조회되면 예외 발생
//        2. fetch() : 여러 건 조회, 리스트를 반환하고 결과가 없으면 빈 리스트를 반환

//        3. fetchFirst() : 여러 건 조회되어도 하나의 결과만 반환
        Employee employee1 = queryFactory.selectFrom(employee)
                .fetchFirst();
        System.out.println("employee1 = " + employee1);

//        4. fetchCount() : 카운트 쿼리로 변경해서 실행 (deprecated)
        long count = queryFactory.selectFrom(employee)
                .fetchCount();
        System.out.println("count = " + count);

//        5. fetchResults() : 페이징 처리 결과 반환, total쿼리도 자동 생성하여 실행한다.(deprecated)
    ...
    void returnType(){
//        반환타입
//        1. 엔티티 타입 : select()에 Q타입을 사용하면 자동으로 해당 엔티티 타입이 반환됨
